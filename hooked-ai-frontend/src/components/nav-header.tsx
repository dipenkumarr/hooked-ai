        ...
        <div className="container flex h-16 items-center justify-between px-4 py-2">
            <Link href="/dashboard" className="flex items-center">
                <div className="font-sans text-xl font-medium tracking-tight">
                    <span className="text-foreground">hooked</span>
            ...
            <div className="flex items-center gap-4">
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        <DropdownMenuLabel>
                            <p className="text-muted-foreground text-xs flex items-center justify-center py-1">{email}</p>
                        </DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem asChild className="text-sm py-1 flex items-center justify-center cursor-pointer">
                            <Link href="/dashboard/billing">Billing</Link>
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
                            onClick={() => signOut({ redirectTo: "/login" })}
                            className="text-destructive cursor-pointer text-sm py-1 flex items-center justify-center"
                        >
                            Sign out
                        </DropdownMenuItem>
                    </DropdownMenuContent>
